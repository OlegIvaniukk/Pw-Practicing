...
test("Place Order Register while Checkout", async ({ validatedPage }) => {
  ...
  await validatedPage.getByRole("link", { name: " Cart" }).click();
  await validatedPage
    .getByText('Proceed To Checkout').click();
  ...
  await validatedPage.locator('textarea[name="message"]').fill("Test order message");
  await validatedPage
